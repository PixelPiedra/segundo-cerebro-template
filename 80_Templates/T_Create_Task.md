...
> **Estado Actual**: `INPUT[inlineSelect(option(Nuevo), option(Asignado), option(Esperando), option(Revisión), option(Pruebas), option(Resuelto), option(Cerrado), option(Archivado)):status]`
> **Esperando a**: `INPUT[text:waiting_for]`
> **Desde**: `INPUT[date:waiting_since]` **Hasta**: `INPUT[date:waiting_until]` (llenar si status = Esperando)
## 🔗 Tareas Relacionadas
